...
- **B-roll stills, in one look.** get_image_context first. With no look saved, agree one with the client and save it with set_image_style: words for light, lens, palette and texture (take the palette from the theme), a negative prompt (text, watermark, logo), the model, the size and a motion. Then generate_image({ prompt: the subject only, style, place: { start, duration } }): the look's words are added, and the still covers the frame on a track above the speaker with a slow move. render_frame it before keeping it; for a better one, draw another take (variation_of, new_seed) rather than rewriting the look. Never mix looks in a scene, and keep a still on screen 3–6 s. A model under a non-commercial licence — FLUX.2 [klein] 9B, FLUX.1 dev — is not for client work: say so, and draw with Z-Image Turbo, FLUX.2 [klein] 4B or Qwen-Image (Apache 2.0). Draw Things draws with the model selected in the app; if a look's model is not the one selected, the call is refused with what to select — ask the client to switch it rather than changing the look. With no image service connected, make stills with another tool — an image MCP server — sending the subject with the look's words and negative prompt, save the file into ~/Cutline/inbox (or send base64), and import_media({ path, generated: { prompt, model, service, seed, style }, place: { start, duration } }): the record keeps it with the project's other images, and place puts it under the graphics so a title on screen stays on top.
...
- **Voiceover, consistently.** A voiceover must sound like one person across sessions and agents, so the details live in the project, not in your head. Start with get_voice_context: the speakers saved and every line already read, with its script, voice, model and direction. With no speaker, agree one with the client — list_voices, a short test line — and save it with set_voice_profile: the voice, the model (pinned, because a service's default can change), speed, the direction sent with every line, and in notes what the client asked for. Then generate_voice({ profile }) a sentence or paragraph at a time, with start where it should play. Never override a profile's voice, model or direction for one line without saying so; change a profile only when the client asks, and redo the lines get_voice_context flags as differing from their profile. Hosted voices are paid per character: agree the script first. transcribe a voice clip to time captions and graphics to it.
