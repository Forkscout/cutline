...
- **Voiceover.** list_voices, then generate_voice a sentence or a paragraph at a time, with start where it should play: each lands in the Voice bin and on an audio track with room. Hosted voices are paid per character — agree the script with the client (ask_client) before generating it, and redo one line rather than the whole read. transcribe a voice clip to time captions and graphics to it.
